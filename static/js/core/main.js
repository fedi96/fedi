...
function play(id,srcx,type,pk,chn){
	...
	var src = 'https://io.streams.rip/'+srcx;chat = chat+getx('league');}
	//var source = type == 'dash' ? {dash: src} : {hls: src};
	var source = type == 'dash' ? {type:"application/dash+xml",src:srcx} : {type:"application/x-mpegurl",src:src};
	$("#loader").remove();
...
/*$( window ).on( "load", function() {
	var th = Cookies.get('theme') ? Cookies.get('theme') : 'off';
